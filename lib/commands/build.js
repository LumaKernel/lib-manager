...
const { removeSync, writeFileSync, mkdirsSync, copySync, existsSync, readFileSync, readdirSync, renameSync } = _fsExtra2.default;
...
function autoRemove(data) {
  if (data && typeof data === 'string') {
    ...
    readdirSync(dir).filter(file => new RegExp(String.raw`^${libmanPrefix}.*\.md$`).test(file)).forEach(file => {
